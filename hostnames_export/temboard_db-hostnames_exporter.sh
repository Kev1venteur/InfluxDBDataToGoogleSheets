...
recTemHosts=$(psql -h u3recu523 -U postgres -p 5433 -d temboard -c 'SELECT hostname FROM monitoring.hosts ORDER BY hostname ASC;')
...
devTemHosts=$(psql -h u3antu505 -U postgres -p 5433 -d temboard -c 'SELECT hostname FROM monitoring.hosts ORDER BY hostname ASC;')
# Remove first second line and the 2 last lines
...
prodTemHosts=$(psql -h u3antu579 -U postgres -p 5433 -d temboard -c 'SELECT hostname FROM monitoring.hosts ORDER BY hostname ASC;')
# Remove first second line and the 2 last lines
