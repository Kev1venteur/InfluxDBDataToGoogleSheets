...
echo "${recTemHosts}" | sed -e '1d;2d;' | head -n -2 > "csv/rec-temboard-hostnames.csv"
...
# Remove first second line and the 2 last lines
echo "${devTemHosts}" | sed -e '1d;2d;' | head -n -2 > "csv/dev-temboard-hostnames.csv"
...
# Remove first second line and the 2 last lines
echo "${prodTemHosts}" | sed -e '1d;2d;' | head -n -2 > "csv/prod-temboard-hostnames.csv"
